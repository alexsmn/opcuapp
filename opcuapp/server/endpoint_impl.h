...
class EndpointImpl : public std::enable_shared_from_this<EndpointImpl> {
 public:
  ...
  void set_status_handler(Endpoint::StatusHandler handler) { status_handler_ = std::move(handler); }
...
// WARNING: Supresses errors.
  ...
  struct Registry {
    std::mutex mutex;
    std::map<OpcUa_Handle /*endpoint*/, std::shared_ptr<EndpointImpl>> endpoints;
  };

  static Registry& registry();

  static void AddEndpoint(OpcUa_Handle endpoint_handle, std::shared_ptr<EndpointImpl> endpoint);
  static void RemoveEndpoint(OpcUa_Handle endpoint_handle);
  static std::shared_ptr<EndpointImpl> GetEndpoint(OpcUa_Handle endpoint_handle);
  ...
  using Sessions = std::map<NodeId /*authentication_token*/, std::shared_ptr<Session>>;
  Sessions sessions_;

  unsigned next_session_id_ = 1;
};

inline EndpointImpl::EndpointImpl(OpcUa_Endpoint_SerializerType serializer_type) {
...
inline void EndpointImpl::Open(String       url,
                    bool                    listen_on_all_interfaces,
                    const OpcUa_ByteString& server_certificate,
                    const OpcUa_Key&        server_private_key,
                    const OpcUa_Void*       pki_config,
  ...
  Check(::OpcUa_Endpoint_Open(
      ...
      const_cast<Endpoint::SecurityPolicyConfiguration*>(security_policies.data())));
}

inline std::vector<const OpcUa_ServiceType*> EndpointImpl::MakeSupportedServices() const {
...
// static
inline void EndpointImpl::BeginInvoke(OpcUa_GetEndpointsRequest& request, const std::function<void(GetEndpointsResponse& response)>& callback) {
...
// static
inline void EndpointImpl::BeginInvoke(OpcUa_FindServersRequest& request, const std::function<void(FindServersResponse& response)>& callback) {
...
// static
inline void EndpointImpl::BeginInvoke(OpcUa_CreateSessionRequest& request, const std::function<void(CreateSessionResponse& response)>& callback) {
...
template<class Response>
inline void EndpointImpl::SendResponse(OpcUa_Handle& context, const OpcUa_RequestHeader& request_header, Response&& response) {
...
inline void EndpointImpl::SendFault(OpcUa_Handle& context, const OpcUa_RequestHeader& request_header, ResponseHeader&& response_header) {
...
inline std::shared_ptr<Session> EndpointImpl::CreateSession(String session_name) {
  std::lock_guard<std::mutex> lock{mutex_};
...
inline std::shared_ptr<Session> EndpointImpl::GetSession(const NodeId& authentication_token) {
  std::lock_guard<std::mutex> lock{mutex_};
...
// static
inline void EndpointImpl::AddEndpoint(OpcUa_Handle endpoint_handle, std::shared_ptr<EndpointImpl> endpoint) {
  std::lock_guard<std::mutex> lock{registry().mutex};
...
// static
inline std::shared_ptr<EndpointImpl> EndpointImpl::GetEndpoint(OpcUa_Handle endpoint_handle) {
  std::lock_guard<std::mutex> lock{registry().mutex};
