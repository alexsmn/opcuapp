...
#define OPCUA_DEFINE_METHODS(Name)              \
  inline void Initialize(OpcUa_##Name& value) { \
    OpcUa_##Name##_Initialize(&value);          \
                                                ...
                                                \
  inline void Clear(OpcUa_##Name& value) { OpcUa_##Name##_Clear(&value); }
...
#define OPCUA_DEFINE_MEMBERS(Name) OPCUA_DEFINE_MEMBERS_EX(Name, OpcUa_##Name)
...
#define OPCUA_DEFINE_STRUCT(Name) OPCUA_DEFINE_STRUCT_EX(Name, OpcUa_##Name)
...
#define OPCUA_DEFINE_ENCODEABLE(Name)                                    \
  struct Name : OpcUa_##Name {                                           \
    ...
    OPCUA_DEFINE_MEMBERS(Name)                                           \
                                                                         \
    static const OpcUa_EncodeableType& type() {                          \
 ...
 * OpcUa_NodeId_CopyTo
 *===========================================================================*/
inline OpcUa_StatusCode OpcUa_NodeId_CopyTo(const OpcUa_NodeId* a_pSource,
                                            OpcUa_NodeId* a_pDestination) {
  ...
  switch (a_pSource->IdentifierType) {
    ...
    case OpcUa_IdentifierType_String: {
      OpcUa_String_StrnCpy(&a_pDestination->Identifier.String,
                           &a_pSource->Identifier.String,
                           OPCUA_STRING_LENDONTCARE);
      break;
    }
    case OpcUa_IdentifierType_Opaque: {
      a_pDestination->Identifier.ByteString.Length =
          a_pSource->Identifier.ByteString.Length;

      if (a_pDestination->Identifier.ByteString.Length > 0) {
        a_pDestination->Identifier.ByteString.Data =
            (OpcUa_Byte*)OpcUa_Alloc(a_pSource->Identifier.ByteString.Length);
        OpcUa_GotoErrorIfAllocFailed(
            a_pDestination->Identifier.ByteString.Data);
        OpcUa_MemCpy(a_pDestination->Identifier.ByteString.Data,
                     a_pDestination->Identifier.ByteString.Length,
                     a_pSource->Identifier.ByteString.Data,
                     a_pSource->Identifier.ByteString.Length);
      } else {
    ...
    case OpcUa_IdentifierType_Guid: {
      if (a_pSource->Identifier.Guid != OpcUa_Null) {
        a_pDestination->Identifier.Guid =
            (OpcUa_Guid*)OpcUa_Alloc(sizeof(OpcUa_Guid));
        OpcUa_GotoErrorIfAllocFailed(a_pDestination->Identifier.Guid);
        OpcUa_Guid_CopyTo(a_pSource->Identifier.Guid,
                          a_pDestination->Identifier.Guid);
      } else {
