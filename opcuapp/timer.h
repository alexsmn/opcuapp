...
class Timer {
 public:
  ...
  void Start(WaitHandler&& handler) {
    core_ = std::make_shared<CoreImpl<WaitHandler>>(
 ...
 private:
  ...
  class CoreImpl : public Core {
   public:
    explicit CoreImpl(WaitHandler&& handler)
        : handler_{std::forward<WaitHandler>(handler)} {}
    ...
    void Start(UInt32 interval_ms) {
      assert(!impl_);
      Check(::OpcUa_Timer_Create(&impl_, interval_ms, &TimerCallback, nullptr,
                                 this));
    }

   private:
    static OpcUa_StatusCode OPCUA_DLLCALL
    TimerCallback(OpcUa_Void* pvCallbackData,
                  OpcUa_Timer hTimer,
                  UInt32 msecElapsed) {
