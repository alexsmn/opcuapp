...
class StdInputStream {
 ...
 private:
  static OpcUa_StatusCode GetPosition(OpcUa_Stream* strm,
  ...
  static OpcUa_StatusCode SetPosition(OpcUa_Stream* strm,
                                      OpcUa_UInt32 position) {
    auto& stream = *reinterpret_cast<std::istream*>(strm->Handle);
  ...
  static OpcUa_StatusCode Read(OpcUa_InputStream* istrm,
                               OpcUa_Byte* buffer,
                               OpcUa_UInt32* count) {
    auto& stream = *reinterpret_cast<std::istream*>(istrm->Handle);
    assert(stream);
    return stream.read(reinterpret_cast<char*>(buffer), *count) ? OpcUa_Good
                                                                : OpcUa_Bad;
  }
...
class VectorOutputStream {
 public:
  explicit VectorOutputStream(std::vector<char>& data) : data_{data} {
 ...
 private:
  ...
  static OpcUa_StatusCode Write(OpcUa_OutputStream* ostrm,
                                OpcUa_Byte* buffer,
                                OpcUa_UInt32 count) {
