...
class Channel {
 public:
  ...
  Channel(OpcUa_Channel_SerializerType serializer_type,
  ...
  using ConnectionStateHandler =
      std::function<void(StatusCode status_code, OpcUa_Channel_Event event)>;
  // WARNING: |context| references must outlive the Channel.
  void Connect(const ChannelContext& context,
               const ConnectionStateHandler& connection_state_handler);
  void Disconnect();

  void Reset();

 private:
  static OpcUa_StatusCode ConnectionStateChangedHelper(
...
inline void Channel::Connect(
  ...
  StatusCode status_code = OpcUa_Channel_BeginConnect(
      handle_, context.url,
      ...
      context.requested_lifetime, context.message_security_mode,
      context.network_timeout_ms, &Channel::ConnectionStateChangedHelper, this);
...
inline void Channel::Disconnect() {
  StatusCode status_code = OpcUa_Channel_BeginDisconnect(
      handle_, &Channel::ConnectionStateChangedHelper, this);
  if (!status_code) {
...
// static
inline OpcUa_StatusCode Channel::ConnectionStateChangedHelper(
    OpcUa_Channel hChannel,
    OpcUa_Void* pCallbackData,
    OpcUa_Channel_Event eEvent,
    OpcUa_StatusCode uStatus) {
