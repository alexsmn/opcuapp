...
class Session {
 public:
  ...
  void Create(const OpcUa_CreateSessionRequest& request,
  ...
  void Close(const OpcUa_CloseSessionRequest& request,
             const CloseCallback& callback);

  using BrowseCallback = std::function<void(StatusCode status_code,
                                            Span<OpcUa_BrowseResult> results)>;
  void Browse(Span<const OpcUa_BrowseDescription> descriptions,
              const BrowseCallback& callback);

  using ReadCallback = std::function<void(StatusCode status_code,
  ...
  void StartPublishing(SubscriptionId subscription_id,
 ...
 private:
  struct Core : public std::enable_shared_from_this<Core> {
    ...
    void Publish();
    void OnPublishResponse(StatusCode status_code,
                           SubscriptionId subscription_id,
                           Span<SequenceNumber> available_sequence_numbers,
                           bool more_notifications,
                           OpcUa_NotificationMessage& notification_message,
                           Span<OpcUa_StatusCode> results);
...
inline void Session::Create(const OpcUa_CreateSessionRequest& request,
                            const CreateCallback& callback) {
  using Request = AsyncRequest<CreateSessionResponse>;
  auto async_request = std::make_unique<Request>(
      [core = core_, callback](CreateSessionResponse& response) {
  ...
  StatusCode status_code = OpcUa_ClientApi_BeginCreateSession(
      core_->channel_.handle(), &request.RequestHeader,
      &request.ClientDescription, &request.ServerUri, &request.EndpointUrl,
      &request.SessionName, &request.ClientNonce, &request.ClientCertificate,
      request.RequestedSessionTimeout, request.MaxResponseMessageSize,
      &Request::OnComplete, async_request.release());
...
inline void Session::Activate(const ActivateCallback& callback) {
  /*{
    ...
    acknowledgements_.insert(acknowledgements_.begin(),
  sent_acknowledgements_.begin(), sent_acknowledgements_.end());
  ...
  auto async_request = std::make_unique<Request>(
      [core = core_, callback](ActivateSessionResponse& response) {
        const StatusCode status_code{response.ResponseHeader.ServiceResult};
        if (status_code)
  ...
  StatusCode status_code = OpcUa_ClientApi_BeginActivateSession(
      core_->channel_.handle(), &request.RequestHeader,
      &request.ClientSignature, request.NoOfClientSoftwareCertificates,
      request.ClientSoftwareCertificates, request.NoOfLocaleIds,
      request.LocaleIds, &request.UserIdentityToken,
      &request.UserTokenSignature, &Request::OnComplete,
...
inline void Session::Close(const OpcUa_CloseSessionRequest& request,
                           const CloseCallback& callback) {
  using Request = AsyncRequest<CloseSessionResponse>;
  auto async_request = std::make_unique<Request>(
      [core = core_, callback](CloseSessionResponse& response) {
        const StatusCode status_code{response.ResponseHeader.ServiceResult};
        if (status_code) {
          std::lock_guard<std::mutex> lock{core->mutex_};
  ...
  StatusCode status_code = OpcUa_ClientApi_BeginCloseSession(
      core_->channel_.handle(), &request.RequestHeader,
      request.DeleteSubscriptions, &Request::OnComplete,
      async_request.release());
...
inline void Session::Browse(Span<const OpcUa_BrowseDescription> descriptions,
                            const BrowseCallback& callback) {
  ...
  auto async_request =
      std::make_unique<Request>([callback](BrowseResponse& response) {
  ...
  StatusCode status_code = OpcUa_ClientApi_BeginBrowse(
      core_->channel_.handle(), &request.RequestHeader, &request.View,
      request.RequestedMaxReferencesPerNode, descriptions.size(),
      descriptions.data(), &Request::OnComplete, async_request.release());
...
inline void Session::Read(Span<const OpcUa_ReadValueId> read_ids,
                          const ReadCallback& callback) {
  ReadRequest request;
  InitRequestHeader(request.RequestHeader);
  ...
  auto async_request =
      std::make_unique<Request>([callback](ReadResponse& response) {
        callback(response.ResponseHeader.ServiceResult,
                 {response.Results, static_cast<size_t>(response.NoOfResults)});
      });

  StatusCode status_code = OpcUa_ClientApi_BeginRead(
      core_->channel_.handle(), &request.RequestHeader, request.MaxAge,
      request.TimestampsToReturn, read_ids.size(), read_ids.data(),
      &Request::OnComplete, async_request.release());
...
inline void Session::Core::InitRequestHeader(
    OpcUa_RequestHeader& header) const {
  std::lock_guard<std::mutex> lock{mutex_};
...
inline void Session::StartPublishing(SubscriptionId subscription_id,
                                     NotificationHandler handler) {
  bool publishing = false;
  {
    std::lock_guard<std::mutex> lock{core_->mutex_};
    assert(core_->subscriptions_.find(subscription_id) ==
           core_->subscriptions_.end());
...
inline void Session::Core::Publish() {
  ...
  auto async_request = std::make_unique<Request>(
      [core = shared_from_this()](PublishResponse& response) {
        core->OnPublishResponse(
            response.ResponseHeader.ServiceResult, response.SubscriptionId,
            {response.AvailableSequenceNumbers,
             static_cast<size_t>(response.NoOfAvailableSequenceNumbers)},
  ...
  StatusCode status_code = OpcUa_ClientApi_BeginPublish(
      channel_.handle(), &request.RequestHeader, acknowledgements.size(),
      acknowledgements.data(), &Request::OnComplete, async_request.release());

  if (!status_code)
    OnError(status_code);
}

inline void Session::Core::OnPublishResponse(
    StatusCode status_code,
    SubscriptionId subscription_id,
    Span<SequenceNumber> available_sequence_numbers,
    bool more_notifications,
    OpcUa_NotificationMessage& message,
    Span<OpcUa_StatusCode> results) {
  if (!status_code) {
    OnError(status_code);
    return;
  ...
  if (handler)
    handler(status_code, {message.NotificationData,
