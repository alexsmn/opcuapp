 ...
 private:
  ...
  static void PushNotification(std::weak_ptr<State> weak_state,
    ...
    if (state->pending.size() >= state->options.max_pending_notifications) {
      ...
      // wire notification (the legacy consumer ignored overflow anyway); we
      // only log the first occurrence so the dropped-notifications signal
      // survives.
  ...
  MonitoredItemCreateResult AddItem(MonitoredItemCreateRequest request) {
    ...
    MonitoredItemId item_id = 0;
    {
      ...
      state_->items.push_back(
          {.monitored_item = monitored_item, .client_handle = client_handle});
    }

    std::weak_ptr<State> weak_state = state_;
    if (request.item_to_monitor.attribute_id == AttributeId::EventNotifier) {
      // Project the event onto the item's EventFilter select clauses right
      // here, where the event meets its field paths, so only the standard wire
      ...
      monitored_item->Subscribe(EventHandler{
          [weak_state, client_handle, field_paths = std::move(field_paths)](
            ...
            PushNotification(weak_state, opcua::EventFieldList{
                                             .client_handle = client_handle,
                                             .event_fields = ProjectEventFields(
                                                 field_paths, event)});
          }});
    } else {
      monitored_item->Subscribe(DataChangeHandler{
          [weak_state, client_handle](const DataValue& value) {
            PushNotification(weak_state, opcua::MonitoredItemNotification{
                                             .client_handle = client_handle,
                                             .value = value});
          }});
    }
